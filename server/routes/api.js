...
// sends to S3
apiRouter.post("/api/user/image", upload.single("image"), isAuthenticated, (req, res) => {
  const image = {
...
apiRouter.delete("/api/user/images", isAuthenticated, (req, res) => {
  db.User.findByIdAndUpdate(req.user.id, {"images": []}).then(res=> {
    res.json();
  }).catch(err=>{
    res.json(err);
  })
